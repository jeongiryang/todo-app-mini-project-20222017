...
app.post('/api/market', async (req, res) => {
  try { const newItem = new Item({ title: req.body.title, deadline: req.body.deadline, type: 'market' }); await newItem.save(); res.json(newItem); }
  catch (err) { res.status(400).json({ message: err.message }); }
});
app.post('/api/todo', async (req, res) => {
  try { const newItem = new Item({ title: req.body.title, importance: req.body.importance, type: 'todo' }); await newItem.save(); res.json(newItem); }
  catch (err) { res.status(400).json({ message: err.message }); }
});
...
if (process.env.NODE_ENV !== 'production') { app.listen(5000, () => console.log(`🚀 서버 실행: http://localhost:5000`)); }
module.exports = app;