...
function GpaPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 flex flex-col min-h-screen relative text-gray-900 dark:text-gray-100 transition-colors">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-3xl md:rounded-[2rem] max-w-3xl w-full shadow-2xl transform transition-all border-4 border-emerald-50 dark:border-gray-700 max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-emerald-50 dark:bg-emerald-900/30 p-5 rounded-2xl border-2 border-emerald-200 dark:border-emerald-800 shadow-inner">
                ...
                <ul className="text-xs font-bold text-gray-700 dark:text-gray-200 space-y-2">
                  ...
                  <li>✅ 3단 대시보드 및 평점 시각화 분석</li>
                  <li>✅ <span className="text-emerald-600 font-black">성적 데이터 다운로드</span> 기능 도입</li>
                </ul>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-gray-700/50 rounded-2xl p-6 mb-6 border border-gray-100 dark:border-gray-600">
              ...
              <div className="space-y-3 text-[11px] md:text-xs px-2">
                <p className="flex items-center gap-3 font-medium bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm"><span className="text-emerald-600 font-black min-w-[45px]">V1.0:</span><span className="text-slate-600 dark:text-gray-400">성적 입력 및 기본적인 학점 계산 기능 구현</span></p>
                <p className="flex items-center gap-3 font-medium bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm"><span className="text-emerald-600 font-black min-w-[45px]">V2.0:</span><span className="text-slate-600 dark:text-gray-400">학기별 탭 인터페이스 및 가이드 투어 도입</span></p>
                <p className="flex items-center gap-3 font-medium bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm"><span className="text-emerald-600 font-black min-w-[45px]">V3.5:</span><span className="text-slate-600 dark:text-gray-400">3단 대시보드 및 전공/전체 성적 분석 로직 강화</span></p>
                <p className="flex items-center gap-3 font-bold bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm"><span className="text-emerald-600 font-black min-w-[45px]">V4.0:</span><span className="text-slate-800 dark:text-gray-200 italic">학점 분석 그래프 시각화 및 모바일 UI 고도화</span></p>
              </div>
            </div>
      ...
      <div className="flex-grow">
        ...
        <div id="tour-dashboard" className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-10 relative z-10">
          <div className="bg-[#111] dark:bg-gray-900 p-6 md:px-10 md:py-10 rounded-3xl md:rounded-[3rem] shadow-2xl border-b-[8px] md:border-b-[10px] border-emerald-900 flex flex-col justify-center items-center text-white">
            <h3 className="text-gray-400 font-black text-xs uppercase mb-2">전체평점 (CGPA)</h3>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 md:p-10 rounded-3xl md:rounded-[3rem] shadow-xl border-2 border-emerald-100 flex flex-col justify-center items-center text-gray-800 dark:text-gray-100">
            <h3 className="text-emerald-600 font-black text-xs uppercase mb-2">전공 평점</h3>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 md:p-10 rounded-3xl md:rounded-[3rem] shadow-xl border-2 border-emerald-50 flex flex-col justify-center items-center text-gray-600 dark:text-gray-300">
            <h3 className="text-gray-400 font-black text-xs uppercase mb-2">최근학기 평점</h3>
        ...
        <form id="tour-form" onSubmit={addCourse} className="bg-white dark:bg-gray-800 p-5 md:p-6 rounded-3xl md:rounded-[2.5rem] shadow-lg border border-gray-100 dark:border-gray-700 grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 mb-8 md:mb-10 items-center relative z-10 w-full">
          <select value={form.semester} onChange={e=>setForm({...form, semester: e.target.value})} className="md:col-span-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-xl font-black text-sm text-gray-700 dark:text-white outline-none border border-gray-100 focus:ring-2 ring-emerald-200 w-full">{SEMESTERS.map(s => <option key={s} value={s}>{s}</option>)}</select>
          ...
          <div className="md:col-span-4 grid grid-cols-2 gap-3 md:gap-4">
            <select value={form.credit} onChange={e=>setForm({...form, credit: e.target.value})} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-xl font-black text-sm text-gray-700 dark:text-white outline-none border border-gray-100">{CREDIT_OPTIONS.map(c => <option key={c} value={c}>{c}학점</option>)}</select>
            ...
            <div className="overflow-x-auto w-full">
              <table className="w-full text-center min-w-[500px] md:min-w-full">
                <thead className="bg-gray-50 dark:bg-gray-900/50 text-gray-400 text-[10px] font-black tracking-widest uppercase border-b border-gray-100">
                  <tr><th className="p-4">분류</th><th className="p-4 text-left">과목명</th><th className="p-4">학점</th><th className="p-4">성적</th><th className="p-4">관리</th></tr>
                </thead>
                <tbody>
                      ...
                      {editingId === course.id ? (
                        <td colSpan="5" className="p-4 bg-yellow-50/50 relative z-10">
                          <div className="flex flex-col sm:flex-row gap-2 items-center p-2 bg-white dark:bg-gray-800 rounded-xl border-2 border-yellow-200">
                            <select value={editForm.semester} onChange={e=>setEditForm({...editForm, semester: e.target.value})} className="w-full sm:w-auto p-2 bg-gray-100 dark:bg-gray-700 rounded-xl font-black text-xs outline-none">{SEMESTERS.map(s => <option key={s} value={s}>{s}</option>)}</select>
                            ...
                            <div className="flex w-full sm:w-auto gap-2">
                              <select value={editForm.credit} onChange={e=>setEditForm({...editForm, credit: e.target.value})} className="flex-1 p-2 bg-gray-100 dark:bg-gray-700 rounded-xl font-black text-xs">{CREDIT_OPTIONS.map(c => <option key={c} value={c}>{c}학점</option>)}</select>
                            ...
                            <div className="flex w-full sm:w-auto gap-2 items-center">
                              <label className="flex items-center gap-1.5 cursor-pointer bg-gray-100 dark:bg-gray-700 px-3 py-2 rounded-xl text-xs"><input type="checkbox" checked={editForm.isMajor} onChange={e=>setEditForm({...editForm, isMajor: e.target.checked})} className="w-3 h-3 accent-emerald-500 rounded cursor-pointer"/><span className="font-black text-gray-600 dark:text-gray-300">전공</span></label>
                              <div className="flex gap-1.5 flex-grow">
                                <button onClick={saveEdit} className="bg-emerald-600 text-white px-4 py-2 rounded-xl font-black text-xs hover:bg-emerald-700 transition shadow-sm flex-grow">저장</button>
                                <button onClick={()=>setEditingId(null)} className="bg-gray-400 text-white px-4 py-2 rounded-xl font-black text-xs hover:bg-gray-500 transition shadow-sm flex-grow">취소</button>
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <>
                          <td className="p-4 flex justify-center items-center h-full"><label className="flex items-center gap-1.5 cursor-pointer bg-gray-50 dark:bg-gray-700 px-3 py-1 rounded-full border border-gray-100"><input type="checkbox" checked={course.isMajor} onChange={e=>{ setCourses(courses.map(c => c.id === course.id ? { ...c, isMajor: e.target.checked } : c)); }} className="w-3 h-3 accent-emerald-500 rounded cursor-pointer"/><span className="font-black text-[10px] text-gray-600 dark:text-gray-300">전공</span></label></td>
                          <td className="p-4 text-left font-black text-gray-800 dark:text-white text-sm md:text-lg">{course.name}</td>
                          <td className="p-4 font-bold text-gray-500 dark:text-gray-400 text-xs">{course.credit}학점</td>
                          <td className="p-4 font-black text-base md:text-xl text-emerald-600">{course.grade}</td>
                          <td className="p-4 flex justify-center gap-2">
                            <button onClick={()=>{setEditingId(course.id); setEditForm(course)}} className="text-[10px] font-black uppercase text-gray-400 hover:text-gray-800 dark:hover:text-white bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-xl transition">수정</button>
                            <button onClick={()=>{if(window.confirm('삭제하시겠습니까?')) setCourses(courses.filter(i=>i.id!==course.id))}} className="text-[10px] font-black uppercase text-red-400 hover:text-red-500 bg-red-50 dark:bg-red-900/30 px-3 py-1.5 rounded-xl transition">Del</button>
                          </td>
                        </>
                      )}
                    </tr>
                  ))}
                  {activeSemesterCourses.length === 0 && <tr><td colSpan="5" className="p-20 text-gray-400 font-bold text-xs">해당 학기에 등록된 성적이 없습니다.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      <footer className="py-8 md:py-12 text-center border-t border-gray-200 mt-8 relative z-10">
